...
void vm_print_mnemonic(const trace_log_t* log, const vm_cmd_t *vm_command)  {
  command_t command;
  ...
  switch(vm_getbits(&command,63,3)) { /* three first bits */
    ...
    case 1: /*  Jump/Call or Link instructions */
      if(vm_getbits(&command,60,1)) {
        ...
        print_if_version_1(log, &command);
        print_link_instruction(log, &command, 0); /*  must be present */
      }
      break;
...
void vm_print_cmd(const trace_log_t* log, int row, const vm_cmd_t *vm_command) {
  int i;
