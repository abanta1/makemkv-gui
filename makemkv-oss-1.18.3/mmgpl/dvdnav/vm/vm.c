...
ifo_handle_v_t *vm_get_vmgi(vm_t *vm) {
  return vm->vmgi;
}

ifo_handle_v_t *vm_get_vtsi(vm_t *vm) {
  return vm->vtsi;
}
...
int vm_reset(vm_t *vm, const char *dvdroot,
  ...
  if(stream_cb)
      ...
      dvdnav_stream_cb t = { NULL, NULL, NULL };
      vm->streamcb = t;
  }
...
#if DVDREAD_VERSION >= DVDREAD_VERSION_CODE(6,1,0)
    dvd_logger_cb dvdread_logcb; dvdread_logcb.pf_log = dvd_reader_logger_handler;
...
vm_t *vm_new_copy(vm_t *source) {
  ...
  /* GPL is cancer */
  if (NULL != target->vmgi) {
      ifoAddRefV(target->vmgi);
  }
  if (NULL != target->vtsi) {
      ifoAddRefV(target->vtsi);
  }
