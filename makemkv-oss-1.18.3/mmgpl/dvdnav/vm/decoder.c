...
static const char *linkcmd2str(link_cmd_t cmd) {
  switch(cmd) {
  case LinkNoLink:
    return "LinkNoLink";
  case LinkTopC:
...
void vm_print_link(const trace_log_t* log, const link_t* value) {
  const char *cmd = linkcmd2str(value->command);
...
static int32_t eval_link_subins(command_t* command, int32_t cond, link_t *return_values) {
  ...
  return_values->command = (link_cmd_t)linkop;
...
int32_t vmEval_CMD(const trace_log_t* log, const vm_cmd_t commands[], int32_t num_commands,
