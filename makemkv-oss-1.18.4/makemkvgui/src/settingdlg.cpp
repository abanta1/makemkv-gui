...
void CSettingDialog::ReadSettings(bool first)
{
    ...
    int auto_submit = client->GetSettingInt(apset_app_AutoSubmitEnabled);
    generalTab->check_AutoSubmit->setCheckState((auto_submit == 0) ? Qt::Unchecked : Qt::Checked);
...
bool CSettingDialog::WriteSettings(bool& restartRequired)
{
    ...
    client->SetSettingInt( apset_app_AutoSubmitEnabled, (generalTab->check_AutoSubmit->checkState() == Qt::Checked) ? 1 : 0 );
...
CGeneralTab::CGeneralTab(QWidget *parent) : QWidget(parent)
{
    ...
    check_AutoSubmit = new QCheckBox();
    net_lay->addWidget(createLabel(UI_QSTRING(APP_IFACE_SETTINGS_ENABLE_AUTO_SUBMIT)), 1, 0, Qt::AlignRight);
    net_lay->addWidget(check_AutoSubmit, 1, 1);
    lineEditProxy = new QLineEdit();
    net_lay->addWidget(createLabel(UI_QSTRING(APP_IFACE_SETTINGS_PROXY_SERVER)), 2, 0, Qt::AlignRight);
    net_lay->addWidget(lineEditProxy,2,1);
